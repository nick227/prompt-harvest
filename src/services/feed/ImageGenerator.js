...
const generateDalleImage = async(prompt, guidance, userId = null) => {
    try {
        ...
        console.log('🎨 DALL-E: Generating image with prompt:', `${prompt.substring(0, 50)}...`);
        ...
        console.log('🎨 DALL-E: Image generated successfully, length:', imageData.length);
...
const makeDezgoRequest = async (url, prompt, model, guidance) => {
    ...
    if (url.includes('text2image_flux')) {
        ...
        console.log(`🔍 DEZGO: Flux request completed in ${duration}ms`);
        ...
        console.log(`🔍 DEZGO: Request completed in ${duration}ms, response size: ${response.data.length} bytes`);
...
const generateImagenImage = async(prompt, guidance, userId = null) => {
    try {
        ...
        console.log('🎨 IMAGEN: Generating image with prompt:', `${prompt.substring(0, 50)}...`);
        ...
        console.log('🎨 IMAGEN: Making request to:', apiEndpoint);
        ...
        if (!prediction.bytesBase64Encoded) {
            console.log('🎨 IMAGEN: No image data in prediction:', JSON.stringify(prediction, null, 2));

            return { error: 'No image data found in response', details: response.data };
        }

        console.log('🎨 IMAGEN: Found base64 image data, length:', prediction.bytesBase64Encoded.length);
...
const generateWithProvider = async(config, prompt, guidance, userId) => {
    ...
    console.log(`🚀 GENERATOR: Starting ${config.type} generation for prompt: "${prompt.substring(0, 30)}..."`);
    ...
    console.log(`⏱️ GENERATOR: ${config.type} generation completed in ${duration}ms`);
...
const generateRandomProviderImage = async(providers, prompt, guidance, userId = null) => {
    ...
    console.log(`🎯 Randomly selected provider: ${selectedProvider} from ${providers.length} candidates`);
