...
import { calculateCost, calculateUserCosts } from '../config/costMatrix.js';
        ...
        try {
            ...
            const transaction = await this.prisma.transaction.create({
                data: {
                    ...
                    provider,
                    count,
                    cost: calculateCost(provider, count),
                    createdAt: new Date()
                }
            });
    ...
    async getProviderUsage(userId, startDate = null, endDate = null) {
        try {
            ...
            const providerBreakdown = usage.map(item => ({
                ...
                totalCost: calculateCost(item.provider) * item._count.provider,
                averageCost: calculateCost(item.provider)
            }));
