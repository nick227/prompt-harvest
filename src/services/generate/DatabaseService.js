...
import { generateId } from '../../utils/GenerateUtils.js';
    ...
    try {
        ...
        return {
            ...
            data: {
                ...
                imageId: image.id,
                imageUrl: imageData.imageUrl,
                userId: image.userId, // Include userId from database record
                isPublic: image.isPublic,
 ...
 */
const saveGenerateEvent = async(type, data, req) => {
    try {
        ...
        if (type === 'prompt') {
            ...
            throw new Error(`Unknown generate event type: ${type}`);
        }
    } catch (error) {
        console.error('❌ Error saving generate event:', error);
        throw error;
    }
};
