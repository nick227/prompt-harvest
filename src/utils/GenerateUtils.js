 ...
 * GenerateUtils
 *
 * Utility functions for the generate system including:
 * - ID generation
 ...
 * the generate system components.
 */
...
const validateImageGenerationParams = params => {
    ...
    if (!Array.isArray(params.providers)) {
        ...
        // Validate individual provider names
        const invalidProviders = params.providers.filter(provider => typeof provider !== 'string' || provider.trim().length === 0
        );

        if (invalidProviders.length > 0) {
            errors.push('All providers must be non-empty strings');
        }
    }

    // Validate guidance (now that we have a fallback, this should always be a number)
    if (typeof params.guidance !== 'number') {
        errors.push('Guidance must be a number');
    } else if (params.guidance < 0 || params.guidance > 20) {
