...
/**
 * Dynamic Railway Deployment Script
 * Handles both local and Railway database structures dynamically
 */
...
async function railwayDeployDynamic() {
    try {
        console.log('🚀 Starting dynamic Railway deployment...');
        ...
        // Step 2: Fix table structure dynamically
        console.log('🔧 Step 2: Fixing table structure dynamically...');
        await fixTableStructureDynamic(tableInfo);

        // Step 3: Insert models with dynamic column handling
        console.log('🤖 Step 3: Inserting models dynamically...');
        await insertModelsDynamic(tableInfo);

        // Step 4: Verify
        console.log('✅ Step 4: Verifying...');
        await verify();

        console.log('🎉 Dynamic Railway deployment successful!');

    } catch (error) {
        console.error('❌ Dynamic deployment failed:', error);
        throw error;
    } finally {
...
async function fixTableStructureDynamic(tableInfo) {
    try {
        console.log('🔧 Fixing table structure dynamically...');
        ...
        try {
            ...
            console.log('✅ Table structure fixed dynamically');
...
async function insertModelsDynamic(tableInfo) {
    try {
        console.log('🗑️ Clearing existing models...');
        await prisma.$executeRaw`DELETE FROM models`;

        console.log('🌱 Inserting models dynamically...');
        ...
        for (const [provider, name, displayName, description, costPerImage, isActive, apiUrl, apiModel, apiSize] of models) {
            try {
                ...
                if (tableInfo.hasProviderId) {
                    // Railway database with providerId column
                    await prisma.$executeRaw`
                    ...
                    // Local database with providerId column
                    await prisma.$executeRaw`
                        INSERT INTO models (id, provider, providerId, name, displayName, description, costPerImage, isActive, apiUrl, apiModel, apiSize, createdAt, updatedAt)
                        VALUES (${id}, ${provider}, ${provider}, ${name}, ${displayName}, ${description}, ${costPerImage}, ${isActive}, ${apiUrl}, ${apiModel}, ${apiSize}, NOW(), NOW())
                    `;
...
// Run the deployment
railwayDeployDynamic()
    .then(() => {
        console.log('🎉 Dynamic Railway deployment completed successfully!');
        process.exit(0);
    })
    .catch((error) => {
        console.error('💥 Dynamic Railway deployment failed:', error);
        process.exit(1);
    });
