...
class WordTypeManager {
    constructor() {
        try {
            ...
            // Don't warm up cache immediately - wait for database to be ready
    ...
    // Method to initialize cache warmup when database is ready
    async initializeCache() {
        ...
        try {
            ...
            try {
                ...
                // Now warm up cache
                console.log('🔥 Initializing WordTypeManager cache...');
                await this.warmupCache();
            } catch (tableError) {
                console.warn('⚠️ word_types table does not exist yet, skipping cache warmup');
                this.isDatabaseReady = false;
                // Don't retry immediately - wait for database schema to be set up
            }
        } catch (error) {
    ...
    async warmupCache(commonWords = []) {
        ...
        if (!this.prisma) {
            console.warn('⚠️ Prisma client not available for cache warmup');
            return;
        }

        if (!this.isDatabaseReady) {
            console.warn('⚠️ Database not ready or WordType table not found, skipping cache warmup');
            return;
        }

        try {
            // Use findMany for bulk query instead of individual queries
            const wordRecords = await this.prisma.word_types.findMany({
                where: {
                    word: {
                        in: commonWords
                    }
            ...
            wordRecords.forEach(record => {
                wordMap.set(record.word, record.types);
            });
            ...
            console.log(`✅ Cache warmup complete. Cache size: ${this.cache ? this.cache.size : 0} (${cachedCount} words cached)`);
        } catch (error) {
