...
class WordTypeManager {
    ...
    async deleteWordType(word) {
        ...
        try {
            ...
            const existingRecord = await this.prisma.word_types.findUnique({
                where: { word: word.toLowerCase() }
            });

            if (!existingRecord) {
                console.log(`Word type "${word}" not found in database, nothing to delete`);

                return { message: 'Word type not found', deleted: false };
            }

            // Delete the record
            const deletedRecord = await this.prisma.word_types.delete({
                where: { word: word.toLowerCase() }
            });

            console.log(`Successfully deleted word type: "${word}"`);
